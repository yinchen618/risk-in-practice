...
async function testStage2Fix() {
	...
	try {
		...
		const eventsResponse = await fetch(
			`http://localhost:8000/api/v1/events?experiment_run_id=${experimentRunId}&limit=5`,
		);
		...
		if (eventsResponse.ok) {
			...
			if (eventsData.data.events.length > 0) {
				...
				eventsData.data.events.slice(0, 3).forEach((event, index) => {
					console.log(
						`   ${index + 1}. ${event.eventId} - ${event.meterId} (Score: ${event.score})`,
					);
				});
			} else {
