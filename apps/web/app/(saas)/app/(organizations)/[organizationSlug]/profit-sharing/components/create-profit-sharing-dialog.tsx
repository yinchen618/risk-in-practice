...
export function CreateProfitSharingDialog({
	...
	useEffect(() => {
		if (debouncedProductSearch && selectedCategory) {
			fetch(
				`/api/organizations/products/search?q=${debouncedProductSearch}&category=${selectedCategory}&organizationId=${organizationId}`,
			)
				.then((res) => res.json())
				.then((data) => setProducts(data.products));
		}
	}, [debouncedProductSearch, selectedCategory, organizationId]);

	// 獲取客戶建議
	useEffect(() => {
		if (debouncedCustomerSearch) {
			fetch(
				`/api/organizations/customers/search?q=${debouncedCustomerSearch}&organizationId=${organizationId}`,
			)
				.then((res) => res.json())
	...
	useEffect(() => {
		...
		if (customerId) {
			fetch(
				`/api/organizations/bank-accounts?customerId=${customerId}&organizationId=${organizationId}`,
			)
				.then((res) => res.json())
				.then((data) => setBankAccounts(data.bankAccounts));
		}
	}, [form.watch("customerId"), organizationId]);
