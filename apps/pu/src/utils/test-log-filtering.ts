...
function testApiLogFiltering() {
	...
	apiLogger.request("http://localhost:8000/api/test", "GET", { test: true });

	console.log("\n2. 測試 API Response 日誌 (應該被過濾):");
	apiLogger.response("http://localhost:8000/api/test", 200, {
		result: "success",
	});

	console.log("\n3. 測試 API Cached 日誌 (應該被過濾):");
	apiLogger.cached("http://localhost:8000/api/test", { fromCache: true });

	console.log("\n4. 測試 API Error 日誌 (應該顯示):");
	apiLogger.error("http://localhost:8000/api/test", new Error("Test error"));
...
function checkLogHistory() {
	...
	recentLogs.forEach((log, index) => {
		console.log(
			`${index + 1}. [${log.level}] ${log.source || "unknown"}: ${log.message}`,
		);
	});
