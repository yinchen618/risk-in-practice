...
class GlobalApiManager {
  ...
  async request<T = any>(
    ...
    // 創建新的請求
    const requestPromise = this.executeRequest<T>(url, options, key, enableCache)
...
export const apiRequest = {
  get: <T = any>(url: string, enableCache = true, enableDeduplication = true) =>
    globalApiManager.request<T>(url, { method: 'GET' }, enableCache, enableDeduplication),
  ...
  delete: <T = any>(url: string, enableDeduplication = true) =>
    globalApiManager.request<T>(url, { method: 'DELETE' }, false, enableDeduplication),
}
