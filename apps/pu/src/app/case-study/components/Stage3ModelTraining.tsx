...
export function Stage3ModelTraining({
	...
	// 移除不需要的 useEffect，因為 experimentRuns 管理已移至父組件
	// useEffect(() => {
	// 	loadExperimentRuns();
	// }, [loadExperimentRuns]);
	...
	return (
		<Card className="border border-blue-200">
			...
			<CardContent className="space-y-6">
				...
				{/* 檢查選中的運行狀態 */}
				{(() => {
					// 這裡可以通過 API 檢查運行狀態，或者從父組件傳入狀態
					// 暫時移除這個檢查，因為 experimentRuns 管理已移至父組件
					// const selectedRun = experimentRuns.find(
					// 	(run) => run.id === selectedRunId,
					// );
					// const isCompleted = selectedRun?.status === "COMPLETED";

					// 暫時假設已完成，實際應該從 API 或父組件獲取狀態
					...
					if (!isCompleted) {
						return (
							<Alert className="bg-amber-50 border-amber-200">
								...
								<AlertDescription>
									The selected dataset must be marked as
									"COMPLETED" before model training can begin.
									Please complete the expert labeling stage
									(Stage 2) first.
									<br />
										...
										<div className="text-center space-y-4">
											...
											{resultsMeta && (
												<div className="mt-6 text-sm text-slate-700 space-y-1">
													...
													<div>
														Type:{" "}
														{resultsMeta.model_type}
													</div>
													<div>
														Created at:{" "}
														{resultsMeta.created_at}
													</div>
												</div>
											)}
										</div>
