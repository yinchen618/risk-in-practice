...
interface D3ParameterChartProps {
  parameterType: 'spike' | 'zscore' | 'duration' | 'timegap' | 'peer' | 'weekendHoliday'
  threshold: number
...
export function D3ParameterChart({
  ...
  const renderSpikeChart = (g: any, width: number, height: number, spikeThreshold: number) => {
    const baseline = 50
    const thresholdValue = baseline * (1 + spikeThreshold / 100)

    // Sample data with spikes
    const dataPoints = [52, 48, 51, 150, 53, 49, 52, 210, 50, 180, 51, 48, 52, 250, 49]
    ...
    const yScale = d3
      .scaleLinear()
      .domain([0, Math.max(Math.max(...dataPoints), thresholdValue) * 1.1])
      .range([height, 0])
  ...
  // A-2. Z-Score Chart
  const renderZScoreChart = (g: any, width: number, height: number, zThreshold: number) => {
    // Generate normal distribution data with some outliers
    const dataPoints = [
      0.2, -0.5, 0.8, 2.8, -0.3, 0.6, -1.2, 3.5, 0.1, -2.1, 0.4, -0.7, 4.2, 0.3, -0.9,
    ]
    ...
    const yScale = d3
      .scaleLinear()
      .domain([Math.min(yExtent[0], -zThreshold) - 0.5, Math.max(yExtent[1], zThreshold) + 0.5])
      .range([height, 0])
  ...
  // B-1. Duration Chart
  const renderDurationChart = (g: any, width: number, height: number, minDuration: number) => {
    const baseline = 30
  ...
  const renderWeekendHolidayChart = (
    ...
    g.append('rect')
      ...
      .attr('height', yScale(weekdayNormal.min) - yScale(weekdayNormal.max))
    ...
    g.append('rect')
      ...
      .attr('height', yScale(holidayNormal.min) - yScale(holidayNormal.max))
    ...
    timeSegments.forEach(segment => {
      ...
      if (
        ...
        (segment.value > holidayNormal.max || segment.value < holidayNormal.min)
      ) {
        isAnomaly = true
        anomalyCount++
      }
  ...
  // C-1. Time Gap Chart
  const renderTimeGapChart = (g: any, width: number, height: number, maxGap: number) => {
    ...
    // Data line with gap
    g.append('path')
      .attr('d', `M 0,${yScale(dataValue)} L ${xScale(30)},${yScale(dataValue)}`)
      .attr('stroke', '#3b82f6')
      .attr('stroke-width', 2)
      .attr('fill', 'none')

    g.append('path')
      .attr('d', `M ${xScale(70)},${yScale(dataValue)} L ${width},${yScale(dataValue)}`)
      .attr('stroke', '#3b82f6')
      .attr('stroke-width', 2)
      .attr('fill', 'none')
  ...
  const renderPeerChart = (g: any, width: number, height: number, exceedThreshold: number) => {
    const peerAverage = 50
    const thresholdValue = peerAverage * (1 + exceedThreshold / 100)

    const dataPoints = [52, 48, 51, 53, 49, 52, 85, 50, 51, 48, 52, 120, 49, 50, 51]
