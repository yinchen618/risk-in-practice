...
import { fetchMeterDataWithTime, getUnitOptionsForBuilding } from '@/hooks/use-testbed-data'
...
export function ExplorerTab() {
  ...
  const setRangeMode = (mode: RangeMode) => setRangeParam(mode === 'custom' ? null : mode)
    ...
    try {
      if (mode === 'single') {
        ...
        if (meter === 'both') {
          ...
          // Single meter type
          const data = await fetchMeterDataWithTime(unit, meter, start, end, startTime, endTime)
          setMeterData(data)
        }
      } else {
        ...
        const roomDataPromises = floorUnits.map(async roomUnit => {
          try {
            if (meter === 'both') {
              ...
              try {
                const applianceData = await fetchMeterDataWithTime(
                  roomUnit.value,
                  'appliance',
                  start,
                  end,
            ...
            return [
              {
                roomNumber: roomUnit.value + (meter === 'appliance' ? 'a' : ''),
                hasAppliance: meter === 'appliance',
                data: roomData,
              },
            ]
          } catch (error) {
            console.error(`Failed to load data for room ${roomUnit.value}:`, error)
            return []
          }
        })

        const results = await Promise.all(roomDataPromises)
        const successfulRoomData = results.flat().filter(result => result !== null)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1">
          <ExplorerControls
            ...
            displayMode={(displayMode as 'single' | 'floor') || 'single'}
        ...
        <div className="lg:col-span-3">
          {(displayMode as 'single' | 'floor') === 'floor' && meterData?.roomData ? (
            ...
            <div className="space-y-6">
              <div className="mb-4">
                ...
                <p className="text-sm text-slate-600">Total {meterData.roomData.length} rooms</p>
              </div>
              {meterData.roomData.map(room => (
                <div key={room.roomNumber} className="border rounded-lg p-4 bg-white shadow-sm">
                  <div className="mb-3">
                    <h4 className="text-md font-medium text-slate-700">
                      Room {room.roomNumber}
                      {room.roomNumber.endsWith('a') ? ' (Appliance Meter)' : ' (Main Meter)'}
                    </h4>
                  </div>
                  <SimpleTimeChart
                    ...
                    error={error}
                    meterData={room.data}
                    selectedMeter={room.roomNumber.endsWith('a') ? 'appliance' : 'main'}
                  />
            ...
            <div className="space-y-6">
              <div className="mb-4">
                ...
                <p className="text-sm text-slate-600">Shows main meter and appliance meter</p>
              </div>
              {meterData.roomData.map(room => (
                <div key={room.roomNumber} className="border rounded-lg p-4 bg-white shadow-sm">
                  <div className="mb-3">
          ...
          <div className="mt-6 text-sm text-slate-600 space-y-2">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="font-medium mb-2">⚠️ Data Indicators:</p>
                <ul className="space-y-1 text-xs">
                  <li>• Red diamonds = potential anomalies</li>
