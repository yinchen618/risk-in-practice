...
export function SimpleTimeChart({
  ...
  useEffect(() => {
    ...
    const data =
      meterData.timeSeries?.map((d: { timestamp: string; power: number }) => ({
    ...
    const applianceRaw =
      meterData.applianceTimeSeries?.map((d: { timestamp: string; power: number }) => ({
        timestamp: parseAsUtcIfNoTz(d.timestamp),
        power: d.power,
      })) || []
    // 如果沒有數據，直接返回
    ...
    const powers = data.map((d: { timestamp: Date; power: number }) => d.power)
    const stdDev = Math.sqrt(
      powers.reduce((sum: number, p: number) => sum + (p - avgPower) ** 2, 0) / powers.length
    )
    ...
    const seriesForExtent = applianceRaw.length > 0 ? data.concat(applianceRaw) : data
    ...
    const dataExtent = d3.extent(data, (d: { timestamp: Date; power: number }) => d.timestamp) as [
      Date,
      Date,
    ]
    const fmtTW = (d: Date) =>
    ...
    console.log('[TimeChart] X domain Taipei:', fmtTW(timeExtent[0]), '~', fmtTW(timeExtent[1]))

    // 針對實際繪圖的資料也輸出範圍與數量（限制在 union）
    const displayData = data.filter(d => d.timestamp >= domainStart && d.timestamp <= domainEnd)
    ...
    // 計算時間範圍來決定適當的格式
    const timeRangeHours = (timeExtent[1].getTime() - timeExtent[0].getTime()) / (1000 * 60 * 60)
    ...
    if (timeRangeHours <= 24) {
      // 小於 24 小時：顯示 小時:分鐘
      timeFormat = 'HM'
      tickCount = Math.min(16, Math.max(6, Math.floor(timeRangeHours / 1.5)))
    } else if (timeRangeHours <= 168) {
      // 小於 7 天：顯示 月/日 小時:分鐘
      timeFormat = 'MDHM'
      tickCount = Math.min(20, Math.max(8, Math.floor(timeRangeHours / 8)))
    } else {
      // 超過 7 天：顯示 月/日
      timeFormat = 'MD'
      tickCount = Math.min(15, Math.max(8, Math.floor(timeRangeHours / 16)))
    }
    ...
    g.selectAll('.dot')
      ...
      .append('circle')
      .attr('class', 'dot')
      .attr('cx', (d: { timestamp: Date; power: number }) => xScale(d.timestamp))
      .attr('cy', (d: { timestamp: Date; power: number }) => yScale(d.power))
      .attr('r', 2.5)
      .style('fill', '#3b82f6')
      .style('stroke', '#fff')
    ...
    if (selectedMeter === 'both' && applianceDisplayData.length > 0) {
      const lineAppliance = d3
        ...
        .defined((d, i) => {
          ...
          const prevTime = applianceDisplayData[i - 1].timestamp.getTime()
          const currentTime = d.timestamp.getTime()
    ...
    g.selectAll('.anomaly')
      ...
      .attr('class', 'anomaly')
      .attr('d', d => {
        const x = xScale((d as { timestamp: Date; power: number }).timestamp)
        const y = yScale((d as { timestamp: Date; power: number }).power)
        const s = 5 // 半徑縮小
    ...
    g.selectAll('.no-data-band')
      ...
      .attr('x', d => {
        const s = new Date(Math.max(timeExtent[0].getTime(), d.start.getTime()))
        return xScale(s)
      })
      .attr('y', height - bandHeight)
      .attr('width', d => {
        const s = new Date(Math.max(timeExtent[0].getTime(), d.start.getTime()))
        const e = new Date(Math.min(timeExtent[1].getTime(), d.end.getTime()))
    ...
    overlay
      .on('mousemove', event => {
        ...
        const bisectDate = d3.bisector((d: { timestamp: Date; power: number }) => d.timestamp).left
        const i = bisectDate(displayData, x0, 1)
        const d0 = displayData[i - 1]
        const d1 = displayData[i]
        const d =
          d1 && x0.getTime() - d0.timestamp.getTime() > d1.timestamp.getTime() - x0.getTime()
            ? d1
            : d0
        ...
        {chartLoading ? (
          <div className="flex items-center justify-center h-[450px]">
            <div className="text-center">
              ...
              <p className="mt-4 text-slate-600">Loading measurement data...</p>
            </div>
          </div>
        ) : error ? (
          <div className="flex items-center justify-center h-[450px]">
          ...
          <div className="flex items-center justify-center h-[450px]">
            <p className="text-slate-500">Please select a unit and date range to view data</p>
          </div>
        )}
      </CardContent>
    </Card>
