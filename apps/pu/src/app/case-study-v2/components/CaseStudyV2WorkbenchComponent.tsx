...
export function CaseStudyV2WorkbenchComponent() {
  ...
  useEffect(() => {
    if (selectedRun && selectedRunId !== 'new') {
      ...
      if (selectedRun.status === 'CONFIGURING' && activeStage !== 'stage-1') {
        setActiveStage('stage-1')
        // updateURL(selectedRunId, "stage-1");
      } else if (selectedRun.status === 'LABELING' && activeStage === 'stage-1') {
  ...
  const stages = [
    ...
    },
    {
      ...
      enabled:
        ...
        (selectedRun?.positive_label_count && selectedRun.positive_label_count > 0),
    },
    {
      id: 'stage-4',
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <Alert variant="destructive">
          <AlertDescription>Error loading experiment run: {runError.message}</AlertDescription>
        </Alert>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-6">
      ...
      <div className="space-y-2">
        <h1 className="text-3xl font-bold tracking-tight">PU Learning Workbench v2</h1>
        <p className="text-muted-foreground">
          Advanced experimental workbench for PU Learning research with dynamic four-stage workflow
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select or create an experiment run to begin the research workflow
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {selectedRunId !== 'new' && selectedRun && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Track your progress through the four-stage research workflow
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                return (
                  <div key={stage.id} className="flex items-center">
                    <Button
                      ...
                      onClick={() => stage.enabled && handleStageChange(stage.id)}
            ...
            <div className="flex items-center gap-4 text-sm">
              <div className="flex items-center gap-2">
                <Badge variant={selectedRun.status === 'CONFIGURING' ? 'default' : 'secondary'}>
                  {selectedRun.status}
                </Badge>
              ...
              {selectedRun.candidate_count !== null && (
                <div className="flex items-center gap-2">
                  <Badge variant="outline">{selectedRun.candidate_count}</Badge>
              ...
              {selectedRun.positive_label_count !== null && (
                <div className="flex items-center gap-2">
                  <Badge variant="outline">
                    {selectedRun.positive_label_count}/{selectedRun.candidate_count} labeled
                  </Badge>
      ...
      {/* Main Content Area */}
      {selectedRunId !== 'new' ? (
        <Tabs value={activeStage} onValueChange={handleStageChange} className="space-y-4">
          ...
          <TabsContent value="stage-3" className="space-y-4">
            {selectedRun && <Stage3TrainingWorkbench experimentRun={selectedRun} />}
          </TabsContent>

          <TabsContent value="stage-4" className="space-y-4">
            {selectedRun && history && <Stage4ResultsAnalysis experiments={[history]} />}
          </TabsContent>
        </Tabs>
      ) : (
        <Card>
          <CardHeader>
            <CardTitle>Welcome to PU Learning Workbench v2</CardTitle>
            <CardDescription>
              Create a new experiment run or select an existing one to begin your research workflow
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground">
              This workbench provides a structured four-stage approach to PU Learning research:
            </p>
            <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
              <li>
                • <strong>Stage 1:</strong> Generate anomaly candidates from raw data using
                configurable filtering rules
              </li>
              <li>
                • <strong>Stage 2:</strong> Review and label candidates to establish ground truth
              </li>
              <li>
                • <strong>Stage 3:</strong> Train and evaluate PU Learning models with real-time
                monitoring
              </li>
              <li>
                • <strong>Stage 4:</strong> Analyze results and compare different experimental
                approaches
              </li>
            </ul>
          </CardContent>
