...
import type { EvaluationRun, ExperimentRun, TrainedModel } from '../types/index'
...
const METRIC_EXPLANATIONS = {
  ...
  'Est. Pos in U': '估計未標記樣本集合中正例的比例，反映 PU 學習算法對數據分布的理解程度。',
  'Class Prior (π)':
    '類別先驗機率，表示正例在整個數據集中的真實比例。這是 PU 學習的重要參數，通常需要領域專家估計。',

  // Training Process Information
  'Total Epochs': '模型完成的總訓練輪數，每個 epoch 表示模型看過完整數據集一次。',
  'Early Stop': '是否觸發早停機制。早停可防止過擬合，當驗證性能不再改善時自動停止訓練。',
  'Converged @': '模型收斂的訓練輪數，即達到最佳驗證性能的 epoch。',
  'Training Time': '模型訓練的總耗時，包括數據處理和梯度更新的時間。',

  // Validation Performance
  'Best Val F1': '驗證集上達到的最佳 F1-Score，用於模型選擇和早停判斷。',
  'Best Val Prec': '驗證集上達到的最佳精確率，反映模型在驗證階段的預測準確性。',
  'Best Val Recall': '驗證集上達到的最佳召回率，反映模型在驗證階段的發現能力。',
...
function TrainingMetrics({ trainingMetrics }: { trainingMetrics: string | object }) {
  return (
    <div className="border-t bg-indigo-50 p-3">
      <div className="text-sm font-medium mb-2 text-indigo-700">Training Performance Metrics</div>{' '}
      {(() => {
        try {
          const metrics =
            typeof trainingMetrics === 'string' ? JSON.parse(trainingMetrics) : trainingMetrics
          ...
          return (
            <div className="bg-white border border-indigo-200 rounded p-3 text-xs space-y-4">
              ...
              {(metrics.nnpu_method || metrics.training_method) && (
                <div className="flex items-center justify-between pb-2 border-b border-gray-200">
                  <span className="font-semibold text-gray-700">Algorithm:</span>
                  <span className="px-2 py-1 bg-purple-100 text-purple-800 rounded text-xs font-medium">
                    {metrics.nnpu_method || metrics.training_method || 'nnPU Learning'}
                  </span>
                </div>
              )}
              ...
              {(metrics.final_test_f1_score !== undefined ||
                ...
                <div>
                  <div className="font-medium text-gray-700 mb-2">🧪 Test Set Performance:</div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {metrics.final_test_f1_score !== undefined && (
                      <div className="bg-green-50 p-2 rounded border border-green-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Test F1']}
                          className="text-green-700 font-medium text-xs"
                        >
                          <span>Test F1</span>
                        </MetricTooltip>
                        <div className="text-lg font-bold text-green-800">
                          {(metrics.final_test_f1_score * 100).toFixed(1)}%
                        </div>
                      </div>
                    )}
                    {metrics.final_test_precision !== undefined && (
                      <div className="bg-blue-50 p-2 rounded border border-blue-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Test Precision']}
                          className="text-blue-700 font-medium text-xs"
                        >
                          <span>Test Precision</span>
                        </MetricTooltip>
                        <div className="text-lg font-bold text-blue-800">
                          {(metrics.final_test_precision * 100).toFixed(1)}%
                        </div>
                      </div>
                    )}
                    {metrics.final_test_recall !== undefined && (
                      <div className="bg-pink-50 p-2 rounded border border-pink-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Test Recall']}
                          className="text-pink-700 font-medium text-xs"
                        >
                          <span>Test Recall</span>
                        </MetricTooltip>
                        <div className="text-lg font-bold text-pink-800">
                          {(metrics.final_test_recall * 100).toFixed(1)}%
                        </div>
                      </div>
                    )}
                    {metrics.final_test_nnpu_risk !== undefined && (
                      <div className="bg-orange-50 p-2 rounded border border-orange-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['nnPU Risk']}
                          className="text-orange-700 font-medium text-xs"
                        >
                          <span>nnPU Risk</span>
                        </MetricTooltip>
                        <div className="text-lg font-bold text-orange-800">
                          {Number(metrics.final_test_nnpu_risk).toFixed(4)}
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {(metrics.true_positive_recall_test !== undefined ||
                metrics.estimated_positive_rate_in_unlabeled !== undefined ||
                metrics.class_prior_used !== undefined) && (
                <div>
                  <div className="font-medium text-gray-700 mb-2">🧬 PU Learning Specifics:</div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                    {metrics.true_positive_recall_test !== undefined && (
                      <div className="bg-teal-50 p-2 rounded border border-teal-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['True Pos Recall']}
                          className="text-teal-700 font-medium text-xs"
                        >
                          <span>True Pos Recall</span>
                        </MetricTooltip>
                        <div className="text-lg font-bold text-teal-800">
                          {(metrics.true_positive_recall_test * 100).toFixed(1)}%
                        </div>
                      </div>
                    )}
                    {metrics.estimated_positive_rate_in_unlabeled !== undefined && (
                      <div className="bg-cyan-50 p-2 rounded border border-cyan-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Est. Pos in U']}
                          className="text-cyan-700 font-medium text-xs"
                        >
                          <span>Est. Pos in U</span>
                        </MetricTooltip>
                        <div className="text-lg font-bold text-cyan-800">
                          {(metrics.estimated_positive_rate_in_unlabeled * 100).toFixed(1)}%
                        </div>
                      </div>
                    )}
                    {metrics.class_prior_used !== undefined && (
                      <div className="bg-indigo-50 p-2 rounded border border-indigo-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Class Prior (π)']}
                          className="text-indigo-700 font-medium text-xs"
                        >
                          <span>Class Prior (π)</span>
                        </MetricTooltip>
                        <div className="text-lg font-bold text-indigo-800">
                          {(metrics.class_prior_used * 100).toFixed(1)}%
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {(metrics.total_epochs_trained !== undefined ||
                ...
                <div>
                  <div className="font-medium text-gray-700 mb-2">⚙️ Training Process:</div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {metrics.total_epochs_trained !== undefined && (
                      <div className="bg-purple-50 p-2 rounded border border-purple-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Total Epochs']}
                    ...
                    {metrics.early_stopped !== undefined && (
                      ...
                      >
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Early Stop']}
                          className={`font-medium text-xs ${
                            metrics.early_stopped ? 'text-orange-700' : 'text-gray-700'
                          }`}
                        >
                        ...
                        <div
                          className={`text-lg font-bold ${
                            metrics.early_stopped ? 'text-orange-800' : 'text-gray-800'
                          }`}
                        >
                          {metrics.early_stopped ? '✓ Yes' : '✗ No'}
                        </div>
                      </div>
                    )}
                    {metrics.convergence_epoch !== undefined && (
                      <div className="bg-green-50 p-2 rounded border border-green-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Converged @']}
                          className="text-green-700 font-medium text-xs"
                        >
                          <span>Converged @</span>
                        </MetricTooltip>
                    ...
                    {metrics.training_time_seconds !== undefined && (
                      <div className="bg-gray-50 p-2 rounded border border-gray-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Training Time']}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {metrics.best_val_f1_score !== undefined && (
                      <div className="bg-emerald-50 p-2 rounded border border-emerald-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Best Val F1']}
                          className="text-emerald-700 font-medium text-xs"
                        >
                          <span>Best Val F1</span>
                        </MetricTooltip>
                        <div className="text-lg font-bold text-emerald-800">
                          {(metrics.best_val_f1_score * 100).toFixed(1)}%
                        </div>
                      </div>
                    )}
                    {metrics.best_val_precision !== undefined && (
                    ...
                    {metrics.best_epoch !== undefined && (
                      <div className="bg-violet-50 p-2 rounded border border-violet-200">
                        <MetricTooltip
                          explanation={METRIC_EXPLANATIONS['Best Epoch']}
...
function TrainingModelConfiguration({ modelConfig }: { modelConfig: string | object }) {
  return (
    <div className="border-t bg-blue-50 p-3">
      <div className="text-sm font-medium mb-2 text-blue-700">Model Configuration</div>
        ...
        try {
          ...
          return (
            <div className="bg-white border border-blue-200 rounded p-3 text-xs space-y-3">
              ...
              {(config.modelType || config.classPrior !== undefined) && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {config.modelType && (
                    <div className="bg-indigo-50 p-2 rounded border border-indigo-200">
                      <div className="text-indigo-700 font-medium">Model Type</div>
                      <div className="font-bold text-indigo-800">{config.modelType}</div>
                    </div>
                  )}

                  {config.classPrior !== undefined && (
                    <div className="bg-green-50 p-2 rounded border border-green-200">
                      <div className="text-green-700 font-medium">Class Prior</div>
                      <div className="font-bold text-green-800">
                        {Number(config.classPrior).toFixed(4)}
                      </div>
                    </div>
                  )}
                </div>
              ...
              {(config.epochs ||
                ...
                <div className="space-y-2">
                  <div className="font-medium text-gray-700 text-xs">Training Parameters:</div>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs">
                    {config.epochs && (
                      <div className="bg-purple-50 p-2 rounded text-center border border-purple-200">
                        <div className="text-purple-700 font-medium">Epochs</div>
                        <div className="font-bold text-purple-800">{config.epochs}</div>
                      </div>
                    )}

                    {config.batchSize && (
                      <div className="bg-orange-50 p-2 rounded text-center border border-orange-200">
                        <div className="text-orange-700 font-medium">Batch Size</div>
                        <div className="font-bold text-orange-800">{config.batchSize}</div>
                      </div>
                    )}

                    {config.learningRate && (
                      <div className="bg-teal-50 p-2 rounded text-center border border-teal-200">
                        <div className="text-teal-700 font-medium">Learning Rate</div>
                        <div className="font-bold text-teal-800">
                          {Number(config.learningRate).toExponential(2)}
                        </div>
                      </div>
                    )}
                    ...
                    {config.earlyStopping !== undefined && (
                      <div className="bg-red-50 p-2 rounded text-center border border-red-200">
                        <div className="text-red-700 font-medium">Early Stopping</div>
                    ...
                    {config.patience && (
                      <div className="bg-pink-50 p-2 rounded text-center border border-pink-200">
                        <div className="text-pink-700 font-medium">Patience</div>
                        <div className="font-bold text-pink-800">{config.patience}</div>
                      </div>
                    )}
                  </div>
                </div>
...
function TrainingModelEvaluations({ modelEvaluations }: { modelEvaluations: EvaluationRun[] }) {
  return (
    <div className="border-t bg-gray-50 p-3">
      ...
      <div className="space-y-2">
        {modelEvaluations.map(evaluation => (
          <div key={evaluation.id} className="bg-white border rounded p-2 text-xs">
            ...
            {evaluation.evaluationMetrics && (
              <div className="mt-1 p-1 bg-gray-50 rounded">
                <pre className="text-xs text-gray-700">
                  {JSON.stringify(
                    typeof evaluation.evaluationMetrics === 'string'
                      ? JSON.parse(evaluation.evaluationMetrics)
                      : evaluation.evaluationMetrics,
                    null,
                    2
                  )}
...
function TrainingDataInformation({ trainingDataInfo }: { trainingDataInfo: any }) {
  return (
    <div className="border-t bg-gray-50 p-3">
      <div className="text-sm font-medium mb-2 text-gray-700">
      ...
      <div className="bg-white border border-gray-200 rounded p-3 text-xs space-y-4">
        ...
        {trainingDataInfo?.p_data_sources && trainingDataInfo?.u_data_sources && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            {/* P Data Sources */}
            <div>
              <div className="font-semibold text-orange-700 mb-1">Positive (P) Data Sources:</div>
              {trainingDataInfo?.p_data_sources?.dataset_ids?.map((datasetId: string) => {
                const datasetName =
                  trainingDataInfo?.p_data_sources?.dataset_names?.[datasetId] ||
                  `Dataset ${datasetId}`
                const datasetInfo = trainingDataInfo?.p_data_sources?.dataset_info?.[datasetId]
                return (
                  <div key={datasetId} className="mb-1 pl-2 border-l-2 border-orange-200">
                    ...
                    {datasetInfo && (
                      <div className="text-gray-600">
                        ...
                        <br />
                        Train: {datasetInfo.train_samples} | Val: {datasetInfo.validation_samples} |
                        Test: {datasetInfo.test_samples}
                      </div>
                    )}
                  </div>
                )
              ...
              <div className="mt-2 pt-2 border-t border-orange-200 font-medium text-orange-800">
                Total P:{' '}
                {trainingDataInfo?.p_data_sources?.total_samples ||
            ...
            {/* U Data Sources */}
            <div>
              <div className="font-semibold text-blue-700 mb-1">Unlabeled (U) Data Sources:</div>
              {trainingDataInfo?.u_data_sources?.dataset_ids?.map((datasetId: string) => {
                const datasetName =
                  trainingDataInfo?.u_data_sources?.dataset_names?.[datasetId] ||
                  `Dataset ${datasetId}`
                const datasetInfo = trainingDataInfo?.u_data_sources?.dataset_info?.[datasetId]
                return (
              ...
              <div className="mt-2 pt-2 border-t border-blue-200 font-medium text-blue-800">
                Total U: {trainingDataInfo?.u_data_sources?.total_samples || 0} samples
              </div>
            </div>
          </div>
        )}
        ...
        {trainingDataInfo.total_samples && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div className="bg-white p-2 rounded border">
              <div className="text-gray-500 text-xs">Total Samples</div>
              <div className="font-semibold text-gray-800">{trainingDataInfo.total_samples}</div>
            </div>
            {trainingDataInfo.train_p_count !== undefined && (
              <div className="bg-orange-50 p-2 rounded border border-orange-200">
                <div className="text-orange-600 text-xs">Training P Samples</div>
                <div className="font-semibold text-orange-800">
                  {trainingDataInfo.actual_train_p_samples || trainingDataInfo.train_p_count}
                </div>
              </div>
            )}
            {trainingDataInfo.train_u_sampled_count !== undefined && (
              <div className="bg-blue-50 p-2 rounded border border-blue-200">
                <div className="text-blue-600 text-xs">Training U Samples</div>
        ...
        {(trainingDataInfo.train_pool_size ||
          ...
          <div className="text-xs text-gray-600 pt-2 border-t border-gray-200">
            ...
            {trainingDataInfo.train_pool_size && `Train: ${trainingDataInfo.train_pool_size}`}
            {trainingDataInfo.validation_pool_size &&
              ` | Val: ${trainingDataInfo.validation_pool_size}`}
            {trainingDataInfo.test_pool_size && ` | Test: ${trainingDataInfo.test_pool_size}`}
            {trainingDataInfo.sampling_method === 'static_subset' && (
            ...
            {trainingDataInfo.u_sample_ratio && (
              <span className="ml-2 px-1 py-0.5 bg-purple-100 text-purple-800 rounded text-xs">
                U Ratio: {(trainingDataInfo.u_sample_ratio * 100).toFixed(0)}%
              </span>
            )}
        ...
        {/* Split Ratios */}
        {(trainingDataInfo.split_ratios || trainingDataInfo.data_split_ratios) && (
          <div className="text-xs text-gray-600">
            <span className="font-medium">Split Ratios:</span>{' '}
            {trainingDataInfo.split_ratios ? (
              <>
                Train {(trainingDataInfo.split_ratios.train * 100).toFixed(0)}% | Val{' '}
                {(trainingDataInfo.split_ratios.validation * 100).toFixed(0)}% | Test{' '}
                {(trainingDataInfo.split_ratios.test * 100).toFixed(0)}%
              </>
            ) : (
              <>
                Train {(trainingDataInfo.data_split_ratios.train_ratio * 100).toFixed(0)}% | Val{' '}
                {(trainingDataInfo.data_split_ratios.validation_ratio * 100).toFixed(0)}% | Test{' '}
                {(trainingDataInfo.data_split_ratios.test_ratio * 100).toFixed(0)}%
              </>
            )}
          </div>
        )}
        ...
        {/* Processing Flags */}
        {(trainingDataInfo?.overlap_removal || trainingDataInfo?.u_sampling_applied) && (
        ...
        {trainingDataInfo.final_training_samples && (
          <div className="pt-2 border-t border-gray-200 bg-green-50 p-2 rounded">
            <div className="text-xs font-medium text-green-800">
              Final Training Set: {trainingDataInfo.final_training_samples} samples
            </div>
          </div>
        )}
      </div>
...
function TrainedModelItem({
  ...
  onRename: (modelId: string, currentName: string, e: React.MouseEvent) => void
    ...
    <div className="border rounded-lg">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <RenameButton onClick={e => onRename(model.id, model.name, e)} title="Rename model" />
            <DeleteButton
            ...
            try {
              ...
              return (
                <div className="mt-2 grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                  ...
                  {metrics.confusion_matrix?.true_negative !== undefined && (
                    <div className="bg-gray-50 p-2 rounded border border-gray-200 text-center">
                      <div className="text-gray-700 font-medium">True Negative</div>
        ...
        {(() => {
          ...
          const getDataSourceInfo = () => {
            try {
              ...
              if (
                ...
                const pDatasets = dataSourceConfig.pDataSources?.datasetIds || []
                const uDatasets = dataSourceConfig.uDataSources?.datasetIds || []
                const totalDatasets = [...new Set([...pDatasets, ...uDatasets])]
              ...
              const splitRatios = dataSourceConfig.splitRatios || dataSourceConfig.split_ratios
              if (splitRatios?.test || splitRatios?.testRatio) {
                const testRatio = (splitRatios.test || splitRatios.testRatio || 0.1) * 100
          ...
          const { trainingSetInfo, testSetInfo } = getDataSourceInfo()
      ...
      {isExpanded && model.training_data_info && (
        <TrainingDataInformation trainingDataInfo={model.training_data_info} />
      )}
...
function TrainedModels({
  ...
  handleDeleteModel: (modelId: string, modelName: string, e: React.MouseEvent) => void
  handleRenameModel: (modelId: string, currentName: string, e: React.MouseEvent) => void
}) {
  return (
    <Card>
      <CardHeader>
        ...
        {trainedModels.length === 0 ? (
          ...
          <div className="space-y-3">
            {trainedModels.map(model => {
              const evaluationCount = getEvaluationCount(model.id)
              const modelEvaluations = getModelEvaluations(model.id)
    ...
    <Card>
      ...
      <CardContent>
        {evaluationRuns.length === 0 ? (
          <div className="text-center text-muted-foreground py-8">
            No evaluation runs yet. Select a trained model and start evaluation.
          </div>
        ) : (
          <div className="space-y-3">
              ...
              return (
                <div key={run.id} className="border rounded-lg">
                  <div
                    ...
                    onClick={() => handleEvaluationToggle(run.id)}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline">{run.scenarioType}</Badge>
                      </div>
                      <div className="flex items-center gap-2">
                        <RenameButton
                          onClick={e => handleRenameEvaluation(run.id, run.name, e)}
                          title="Rename evaluation"
                        />
                        <DeleteButton
                          onClick={e => handleDeleteEvaluation(run.id, run.name, e)}
                        ...
                        <div className="text-xs text-muted-foreground">
                          {run.evaluationMetrics ? (isExpanded ? '▼' : '▶') : ''}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-xs text-gray-600">
                      <div className="flex items-center gap-1 bg-blue-50 px-2 py-1 rounded border border-blue-200">
                        ...
                        <span>{getModelName(run.trainedModelId)}</span>
                      </div>
                      <div className="flex items-center gap-1 bg-orange-50 px-2 py-1 rounded border border-orange-200">
                        <Database className="h-3 w-3 text-orange-600" />
                        <span className="font-medium text-orange-700">Test Set:</span>
                        <span>{getTestSetInfo(run.testSetSource)}</span>
                      </div>
                    </div>
                    ...
                    {run.evaluationMetrics &&
                      (() => {
                        try {
                          const metrics =
                            typeof run.evaluationMetrics === 'string'
                              ? JSON.parse(run.evaluationMetrics)
                              : run.evaluationMetrics

                          if (!metrics || !metrics.precision || !metrics.recall) {
                            return null
                          }
                    ...
                    <div className="border-t bg-gray-50 p-3">
                      ...
                      {(() => {
                        ...
                        if (!metrics || !metrics.precision || !metrics.recall) {
                          return (
                            <div className="text-center text-gray-500 py-4">
                              ⚠️ Incomplete evaluation metrics data
                            </div>
                          )
                        }
                        ...
                        if (!confusionMatrix) {
                          ...
                          tp = fp = fn = tn = 0
                        } else if (Array.isArray(confusionMatrix) && confusionMatrix.length >= 2) {
                          // 2D array format: [[tn, fp], [fn, tp]]
                          if (
                            Array.isArray(confusionMatrix[0]) &&
                            Array.isArray(confusionMatrix[1])
                          ) {
                            ...
                            fn = confusionMatrix[2] || 0
                            tp = confusionMatrix[3] || 0
                          }
                        } else if (typeof confusionMatrix === 'object') {
                          // Object format: {tp: x, fp: x, fn: x, tn: x}
                          tp = confusionMatrix.tp || confusionMatrix.true_positive || 0
                          fp = confusionMatrix.fp || confusionMatrix.false_positive || 0
                          fn = confusionMatrix.fn || confusionMatrix.false_negative || 0
                          tn = confusionMatrix.tn || confusionMatrix.true_negative || 0
                        } else {
                        ...
                        const f1Score = (metrics.f1_score * 100).toFixed(2)
                        const accuracy = (metrics.accuracy * 100).toFixed(2)
                        const aucRoc = (metrics.auc_roc * 100).toFixed(1)
                          ...
                          <div className="mt-4 space-y-6">
                              ...
                              <div className="bg-white border border-gray-300 rounded-lg overflow-hidden">
                                <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
                                  ...
                                  <p className="text-sm text-gray-600 mt-1">
                                    2×2 contingency table for binary classification evaluation
                                  </p>
                                </div>
                                <div className="p-6">
                                  ...
                                  <div className="mt-4 text-center text-sm text-gray-600">
                                    <p className="mb-2">
                                      ...
                                      {(((tp + tn) / (tp + fp + fn + tn)) * 100).toFixed(1)}%
                                    </p>
                                    <p>
                                      <strong>Error Rate:</strong>{' '}
                                      {(((fp + fn) / (tp + fp + fn + tn)) * 100).toFixed(1)}%
                                    </p>
                                  </div>
                                </div>
                              </div>
                            ...
                            <div className="bg-white border border-gray-300 rounded-lg overflow-hidden">
                              <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
                                ...
                                <p className="text-sm text-gray-600 mt-1">
                                  Economic and operational risk analysis based on classification
                                  errors
                                </p>
                              </div>
                              <div className="p-6">
                                  ...
                                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                                    <div className="flex items-center mb-3">
                                      ...
                                      <h5 className="font-semibold text-red-800">
                                        Type II Error Impact (False Negatives)
                                      </h5>
                                    </div>
                                    <div className="mb-3">
                                    ...
                                    <div className="space-y-2 text-sm text-gray-700">
                                      <div className="flex items-center">
                                        ...
                                        <span>Equipment damage risk escalation</span>
                                      </div>
                                      <div className="flex items-center">
                                        <span className="w-2 h-2 bg-red-400 rounded-full mr-2" />
                                        <span>Undetected energy consumption inefficiencies</span>
                                      </div>
                                      <div className="flex items-center">
                                        <span className="w-2 h-2 bg-red-400 rounded-full mr-2" />
                                        <span>Potential safety protocol violations</span>
                                      </div>
                                    </div>
                                  </div>
                                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                    <div className="flex items-center mb-3">
                                      ...
                                      <h5 className="font-semibold text-yellow-800">
                                        Type I Error Impact (False Positives)
                                      </h5>
                                    </div>
                                    <div className="mb-3">
                                    ...
                                    <div className="space-y-2 text-sm text-gray-700">
                                      <div className="flex items-center">
                                        ...
                                        <span>Operator alert desensitization</span>
                                      </div>
                                      <div className="flex items-center">
                                        <span className="w-2 h-2 bg-yellow-400 rounded-full mr-2" />
                                        <span>Unnecessary maintenance resource allocation</span>
                                      </div>
                                      <div className="flex items-center">
                                        <span className="w-2 h-2 bg-yellow-400 rounded-full mr-2" />
                                        <span>System credibility deterioration</span>
                                      </div>
                                    </div>
                                  </div>
                                </div>
...
export function Stage3TrainingWorkbench({ experimentRun }: Stage3TrainingWorkbenchProps) {
  ...
  const [evaluationDataConfig, setEvaluationDataConfig] = useState<EvaluationDataConfig>({
  ...
  useEffect(() => {
    if (trainingLogRef.current) {
      trainingLogRef.current.scrollTop = trainingLogRef.current.scrollHeight
    }
  }, [trainingLogs])

  useEffect(() => {
    if (evaluationLogRef.current) {
      evaluationLogRef.current.scrollTop = evaluationLogRef.current.scrollHeight
    }
  }, [evaluationLogs])
  ...
  const handleRatioChange = (
    type: keyof Pick<DataSourceConfig, 'trainRatio' | 'validationRatio' | 'testRatio'>,
    value: number
  ) => {
    if (type === 'testRatio') {
    ...
    const autoTestRatio = 100 - newDataSourceConfig.trainRatio - newDataSourceConfig.validationRatio
  ...
  const loadTrainedModels = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (scenarioType === 'GENERALIZATION_CHALLENGE' && !selectedSourceModel) {
          const firstERMModel = models.find(
            (m: any) => m.scenarioType === 'ERM_BASELINE' && m.status === 'COMPLETED'
          )
          if (firstERMModel) {
  ...
  const loadAvailableDatasets = async () => {
    try {
      const response = await fetch('https://python.yinchen.tw/api/v2/analysis-datasets')
      if (response.ok) {
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      const runningModels = trainedModels.filter(model => model.status === 'RUNNING')
      const completedModels = trainedModels.filter(model => model.status === 'COMPLETED')
      const failedModels = trainedModels.filter(model => model.status === 'FAILED')
      ...
      if (runningModels.length > 0) {
        ...
        toast.success(`📁 Data loaded: ${modelCount} models, ${evaluationCount} evaluation records`)
      }
    } catch (error) {
      console.error('Failed to reload data:', error)
      toast.error('❌ Failed to reload data, please check network connection')
    }
  }

  // Helper functions for model-evaluation relationships
  const getEvaluationCount = (modelId: string) => {
    return evaluationRuns.filter(run => run.trainedModelId === modelId).length
  }
  ...
  const handleDeleteModel = async (modelId: string, modelName: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch(`https://python.yinchen.tw/api/v2/trained-models/${modelId}`, {
        method: 'DELETE',
      })
  ...
  const handleRenameModel = async (modelId: string, currentName: string, e: React.MouseEvent) => {
    // Prevent event bubbling to the parent div
    e.stopPropagation()

    const newName = prompt(`Enter new name for model "${currentName}":`, currentName)
    ...
    try {
      console.log('🏷️ Rename model request:', modelId, '->', newName.trim())
      ...
      toast.success(`Model successfully renamed from "${currentName}" to "${newName.trim()}"`)
  ...
  const handleDeleteEvaluation = async (
    ...
    try {
      ...
      // Update local state
      setEvaluationRuns(prevRuns => prevRuns.filter(run => run.id !== evaluationId))
  ...
  const handleRenameEvaluation = async (
    ...
    const newName = prompt(`Enter new name for evaluation "${currentName}":`, currentName)
    ...
    try {
      console.log('🏷️ Rename evaluation request:', evaluationId, '->', newName.trim())
      ...
      toast.success(`Evaluation successfully renamed from "${currentName}" to "${newName.trim()}"`)
  ...
  const startTraining = async () => {
    ...
    try {
      ...
      const response = await fetch('https://python.yinchen.tw/api/v2/trained-models', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        console.log('🔍 Full response object:', JSON.stringify(newModel, null, 2))
        ...
        if (newModel.jobId) {
          ...
          console.error('❌ No jobId received, cannot start monitoring')
          setTrainingLogs(prev => [...prev, '❌ No jobId received, cannot start monitoring'])
        }
      } else {
  ...
  const startEvaluation = async () => {
    ...
    const modelToUse =
      scenarioType === 'GENERALIZATION_CHALLENGE' ? selectedSourceModel : selectedModel
    ...
    try {
      ...
      if (scenarioType === 'DOMAIN_ADAPTATION' || scenarioType === 'GENERALIZATION_CHALLENGE') {
      ...
      const response = await fetch('https://python.yinchen.tw/api/v2/evaluation-runs', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const startTrainingMonitor = (jobId: string) => {
    ...
    console.log('💡 Use the reload button to manually check training status')

    // Show notification to user
    toast.info('Training started! Use the reload button to check progress.', {
      duration: 5000,
    })
  }
  ...
  const startEvaluationMonitor = (jobId: string) => {
    ...
    console.log('💡 Use the reload button to manually check evaluation status')

    // Show notification to user
    toast.info('Evaluation started! Use the reload button to check progress.', {
      duration: 5000,
    })
  }
            ...
            <div className="space-y-2 text-sm text-blue-700">
              {filteringParams ? (
                <div className="space-y-2">
                  ...
                  Array.isArray(filteringParams.selectedDatasetIds) &&
                  filteringParams.selectedDatasetIds.length > 0 ? (
                    <div>
                      <strong>Selected Datasets:</strong>{' '}
                      <span className="text-blue-600">
                        {filteringParams.selectedDatasetIds?.length || 0} dataset(s) selected
                      </span>
                      <div className="mt-2 space-y-1">
                        {availableDatasets.length > 0 ? (
                          availableDatasets
                            .filter(dataset =>
                              filteringParams.selectedDatasetIds?.includes(dataset.id)
                            )
                            .map(dataset => (
                              <div
                                key={dataset.id}
                              ...
                              >
                                <div className="font-medium text-blue-800">{dataset.name}</div>
                                <div className="text-blue-600">
                                  {dataset.building} - {dataset.floor} - {dataset.room} |{' '}
                                  {dataset.occupantType}
                                </div>
                                <div className="text-blue-500">
                                  {dataset.totalRecords?.toLocaleString()} records,{' '}
                                  {dataset.positiveLabels} anomalies
                                </div>
                              </div>
                            ))
                        ) : (
                    ...
                    <div className="text-blue-600">
                      <strong>Datasets:</strong> No specific datasets selected (legacy
                      configuration)
                    </div>
                  )}

                  {/* Time Range */}
                  {filteringParams.startDate && filteringParams.endDate && (
                    <div>
                      <strong>Time Range:</strong> {filteringParams.startDate}{' '}
                      {filteringParams.startTime || '00:00'} → {filteringParams.endDate}{' '}
                      {filteringParams.endTime || '23:59'}
                    </div>
                  )}
                  ...
                  <div className="pt-2 border-t border-blue-200">
                    <div>
                      ...
                      {(experimentRun.total_data_pool_size || 0).toLocaleString()}
                    </div>
                    <div>
                      <strong>Positive Labels:</strong> {experimentRun.positive_label_count || 0}
                    </div>
                    <div>
                ...
                <div className="space-y-1">
                  <div>Source: Total Data Pool from Selected Datasets</div>
                  <div>
                    Total Data Pool Size:{' '}
                    {(experimentRun.total_data_pool_size || 0).toLocaleString()}
                  </div>
                  <div>Positive Labels: {experimentRun.positive_label_count || 0}</div>
                  <div>Negative Labels: {experimentRun.negative_label_count || 0}</div>
                  <div>Candidate Events: {experimentRun.candidate_count || 0}</div>
                </div>
              )}
            </div>
          </div>
        ...
        return (
          <div className="space-y-3">
            <div className="space-y-3 p-3 border rounded-lg bg-green-50/30">
              <h5 className="text-sm font-medium text-green-800">🎯 Source Model Selection</h5>
              <div>
                <Label className="text-xs">Select Model for Evaluation</Label>
                <Select value={selectedSourceModel} onValueChange={setSelectedSourceModel}>
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    {trainedModels
                      .filter(m => m.scenarioType === 'ERM_BASELINE' && m.status === 'COMPLETED')
                      .map(model => (
            ...
            <div className="space-y-3 p-3 border rounded-lg bg-orange-50/30">
              ...
              <div>
                <Label className="text-xs">Select Target Dataset for Testing</Label>
                <Select value={selectedTargetDataset} onValueChange={setSelectedTargetDataset}>
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {availableDatasets.map(dataset => (
                      <SelectItem key={dataset.id} value={dataset.id.toString()}>
                        ...
                        {dataset.building_name || dataset.building}{' '}
                        {dataset.floor_name || dataset.floor} {dataset.room_name || dataset.room}
                        {': P'} {dataset.anomaly_count || dataset.positiveLabels || 0}
                        {' / Total '}
                        {dataset.record_count || dataset.totalRecords || 0}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
      ...
      case 'DOMAIN_ADAPTATION':
        return (
          <div className="space-y-3">
            <div className="space-y-3 p-3 border rounded-lg bg-blue-50/30">
              <h5 className="text-sm font-medium text-blue-800">
                📊 Dataset Selection & Filtering Configuration (Source Domain)
              </h5>
            ...
            <div className="space-y-3 p-3 border rounded-lg bg-orange-50/30">
              <h5 className="text-sm font-medium text-orange-800">
                🎯 Target Distribution (Unlabeled & Test Samples)
              </h5>
              <div>
                <Label className="text-xs">Select Target Dataset for Unlabeled Data</Label>
                <Select value={selectedTargetDataset} onValueChange={setSelectedTargetDataset}>
                  <SelectTrigger>
  ...
  const renderModelTrainingConfiguration = () => {
    if (scenarioType !== 'ERM_BASELINE' && scenarioType !== 'DOMAIN_ADAPTATION') {
      return null
    }
    ...
    return (
      <div className="space-y-4">
        {/* PU Learning Strategy */}
        <div className="space-y-3 p-3 border rounded-lg bg-blue-50/30">
          <h5 className="text-sm font-medium text-blue-800">🎯 PU Learning Strategy</h5>

          <div className="space-y-2">
            <Label className="text-xs">Class Prior (π_p): {modelConfig.classPrior}</Label>
            <Slider
            ...
            <p className="text-xs text-blue-700">
              True positive proportion estimate, this is the most important parameter in nnPU
            </p>
          </div>
        </div>
        ...
        {/* Data Preparation */}
        <div className="space-y-3 p-3 border rounded-lg bg-green-50/30">
          <h5 className="text-sm font-medium text-green-800">📊 Data Preparation</h5>

          <div className="space-y-2">
            <Label className="text-xs">
              Window Size (Time Window): {modelConfig.windowSize} minutes
            </Label>
            <Slider
            ...
            <p className="text-xs text-green-700">
              Time series length input to the model, must be sufficient to capture complete
              behavioral patterns
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3 p-3 border rounded-lg bg-purple-50/30">
          <h5 className="text-sm font-medium text-purple-800">🏗️ Model Architecture</h5>
          ...
          <div className="space-y-2">
            <Label className="text-xs">Hidden Layer Size: {modelConfig.hiddenSize}</Label>
            <Slider
          ...
          <div className="space-y-2">
            <Label className="text-xs">Number of Layers: {modelConfig.numLayers}</Label>
            <Slider
          ...
          <div className="space-y-2">
            <Label className="text-xs">Dropout Rate: {modelConfig.dropout}</Label>
            <Slider
        ...
        {/* Training Process */}
        <div className="space-y-3 p-3 border rounded-lg bg-orange-50/30">
          <h5 className="text-sm font-medium text-orange-800">⚡ Training Process</h5>
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="0.005">Medium-Fast (0.005)</SelectItem>
                  <SelectItem value="0.001">Medium (0.001) ✅</SelectItem>
                  <SelectItem value="0.0005">Slow (0.0005)</SelectItem>
                  <SelectItem value="0.0001">Very Slow (0.0001)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="0.00001">Very Light (1e-5)</SelectItem>
                  <SelectItem value="0.0001">Light (1e-4) ✅</SelectItem>
                  <SelectItem value="0.001">Medium (1e-3)</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <Label className="text-xs">Training Epochs: {modelConfig.epochs}</Label>
            <Slider
        ...
        <div className="space-y-3 p-3 border rounded-lg bg-red-50/30">
          <h5 className="text-sm font-medium text-red-800">🛡️ Training Stability</h5>
          ...
          {modelConfig.earlyStopping && (
            <div className="space-y-2 ml-6">
              <Label className="text-xs">Patience: {modelConfig.patience} epochs</Label>
              <Slider
          ...
          <div>
            <Label className="text-xs">Learning Rate Scheduler</Label>
            <Select
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure and execute model training and evaluation as two distinct, monitorable tasks
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <h3 className="font-semibold mb-2">Total Data Pool Distribution</h3>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-semibold">Experiment Status</h3>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={refreshData}
              ...
              <div className="space-y-1 text-sm">
                <div>
                  Current Stage: <Badge>{experimentRun.status}</Badge>
                </div>
                <div>
                  Trained Models: <Badge variant="outline">{trainedModels.length}</Badge>
                </div>
                <div>
                  Evaluation Runs: <Badge variant="outline">{evaluationRuns.length}</Badge>
                </div>
              </div>
            </div>
            <div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Choose your experiment type to configure the appropriate workflow
              </CardDescription>
            </CardHeader>
              ...
              <div className="space-y-2">
                ...
                >
                  <div className="flex items-start space-x-2 rounded-md hover:bg-accent/30 transition-colors">
                    <RadioGroupItem value="ERM_BASELINE" id="erm-baseline" className="mt-1" />
                    <div className="flex-1">
                      <Label htmlFor="erm-baseline" className="cursor-pointer text-sm font-medium">
                        ERM Baseline
                      </Label>
                  ...
                  <div className="flex items-start space-x-2 rounded-md hover:bg-accent/30 transition-colors">
                    ...
                    />
                    <div className="flex-1">
                      <Label htmlFor="gen-challenge" className="cursor-pointer text-sm font-medium">
                  ...
                  <div className="flex items-start space-x-2 rounded-md hover:bg-accent/30 transition-colors">
                    <RadioGroupItem value="DOMAIN_ADAPTATION" id="domain-adapt" className="mt-1" />
                    <div className="flex-1">
                      <Label htmlFor="domain-adapt" className="cursor-pointer text-sm font-medium">
                        Domain Adaptation
                      </Label>
                      <p className="text-xs text-muted-foreground mt-0.5">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {scenarioType === 'ERM_BASELINE' && 'Source data information'}
                {scenarioType === 'GENERALIZATION_CHALLENGE' && 'Model and dataset selection'}
                {scenarioType === 'DOMAIN_ADAPTATION' && 'Source and target domain setup'}
              </CardDescription>
            </CardHeader>
          ...
          {(scenarioType === 'ERM_BASELINE' || scenarioType === 'DOMAIN_ADAPTATION') && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                ...
                <CardDescription>
                  Configure nnPU hyperparameters and training settings
                </CardDescription>
              </CardHeader>
              <CardContent>{renderModelTrainingConfiguration()}</CardContent>
            </Card>
          )}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        <Slider
                          value={[dataSourceConfig.trainRatio]}
                          onValueChange={([value]) => handleRatioChange('trainRatio', value)}
                          max={80}
                          min={50}
                          step={5}
                          className="flex-1"
                        />
                        <span className="text-xs w-8">{dataSourceConfig.trainRatio}%</span>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        <Slider
                          value={[dataSourceConfig.validationRatio]}
                          onValueChange={([value]) => handleRatioChange('validationRatio', value)}
                          max={30}
                          min={10}
                          step={5}
                          className="flex-1"
                        />
                        <span className="text-xs w-8">{dataSourceConfig.validationRatio}%</span>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        <Slider
                          value={[dataSourceConfig.testRatio]}
                          onValueChange={([value]) => handleRatioChange('testRatio', value)}
                          max={30}
                          min={10}
                          step={5}
                          className="flex-1"
                        />
                        <span className="text-xs w-8">{dataSourceConfig.testRatio}%</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-xs text-muted-foreground">
                    ...
                    {dataSourceConfig.trainRatio +
                      dataSourceConfig.validationRatio +
                      dataSourceConfig.testRatio !==
                      100 && <span className="text-red-600 ml-1">(Must equal 100%)</span>}
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-3 border-t border-gray-200">
                  <div>
                    <Label className="text-xs font-medium">Training Data Sampling</Label>
                    <div className="flex items-center space-x-2 mt-1">
                      <Label className="text-xs text-muted-foreground min-w-0">
                        U Sample Ratio:
                      </Label>
                      <Slider
                        value={[dataSourceConfig.uSampleRatio * 100]}
                        onValueChange={([value]) =>
                          setDataSourceConfig({
                            ...dataSourceConfig,
                            uSampleRatio: value / 100,
                          })
                        }
                        max={100}
                        min={5}
                      ...
                      <span className="text-xs w-12 text-center">
                        {(dataSourceConfig.uSampleRatio * 100).toFixed(0)}%
                      </span>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      Percentage of unlabeled (U) samples to use for training. Lower values reduce
                      training time but may affect model performance.
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <div className="space-y-3">
                ...
                {(scenarioType === 'ERM_BASELINE' || scenarioType === 'DOMAIN_ADAPTATION') && (
                  ...
                  >
                    ...
                    <>
                      <Play className="h-4 w-4 mr-2" />▶ Start Model Training
                    </>
                  </Button>
                )}

                {/* Model Selection and Evaluation for ERM_BASELINE */}
                {scenarioType === 'ERM_BASELINE' && trainedModels.length > 0 && (
                  <>
                    <div className="space-y-2">
                      <Label className="text-sm font-medium">Select Model for Evaluation</Label>
                    ...
                    <Button
                      ...
                      disabled={evaluationStatus.isEvaluating || !selectedModel}
                      variant="outline"
                      className="w-full"
                    >
                      {evaluationStatus.isEvaluating ? (
                        ...
                        <>
                          <TrendingUp className="h-4 w-4 mr-2" />📈 Start Model Evaluation
                        </>
                      )}
                    </Button>
                  </>
                ...
                {scenarioType === 'DOMAIN_ADAPTATION' && trainedModels.length > 0 && (
                  <>
                    <div className="space-y-2">
                      <Label className="text-sm font-medium">Select Model for Evaluation</Label>
                      <Select value={selectedModel} onValueChange={setSelectedModel}>
                    ...
                    <div className="space-y-2">
                      <Label className="text-sm font-medium">🎯 Target Dataset for Testing</Label>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          {availableDatasets.map(dataset => (
                            <SelectItem key={dataset.id} value={dataset.id.toString()}>
                              {dataset.building_name || dataset.building}{' '}
                              {dataset.floor_name || dataset.floor}{' '}
                              {dataset.room_name || dataset.room}: P {dataset.positiveLabels || 0} /
                              Total {dataset.record_count || dataset.totalRecords || 0}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    <Button
                      onClick={handleStartEvaluation}
                      disabled={
                        evaluationStatus.isEvaluating || !selectedModel || !selectedTargetDataset
                      }
                      variant="outline"
                      className="w-full"
                    >
                ...
                {scenarioType === 'GENERALIZATION_CHALLENGE' && (
                  <Button
                    onClick={handleStartEvaluation}
                    disabled={evaluationStatus.isEvaluating || !selectedSourceModel}
                    variant="outline"
                    className="w-full"
                  >
                    {evaluationStatus.isEvaluating ? (
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center gap-2">
                    ...
                    Training Monitor
                    {isTraining && <Loader2 className="h-3 w-3 animate-spin text-blue-500" />}
                  </CardTitle>
                  <ConnectionIndicator connected={false} isActive={isTraining} />
                </div>
              </CardHeader>
              <CardContent>
                <div
                ...
                >
                  {trainingLogs.length === 0 ? (
                    <div className="text-gray-500 italic">No training logs yet...</div>
                  ) : (
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center gap-2">
                    ...
                    Evaluation Monitor
                    {isEvaluating && <Loader2 className="h-3 w-3 animate-spin text-blue-500" />}
                  </CardTitle>
                  <ConnectionIndicator connected={false} isActive={isEvaluating} />
                </div>
              </CardHeader>
              <CardContent>
                <div
                ...
                >
                  {evaluationLogs.length === 0 ? (
                    <div className="text-gray-500 italic">No evaluation logs yet...</div>
                  ) : (
