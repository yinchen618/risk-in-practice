...
export function Stage1CandidateGeneration({
  ...
  const [availableDatasets, setAvailableDatasets] = useState<AnalysisDataset[]>([])
  ...
  const selectedDatasets = useMemo(() => {
    return availableDatasets.filter(dataset => filterParams.selectedDatasetIds.includes(dataset.id))
  ...
  useEffect(() => {
    ...
    const filteringParamsData = (initialExperimentRun as any)?.filteringParameters
    ...
    if (filteringParamsData && availableDatasets.length > 0) {
      try {
        ...
        console.log('🔍 Loading saved filtering parameters:', savedParams)
        console.log(
        ...
        setFilterParams(prev => {
          const newParams = {
            ...
            selectedDatasetIds: savedParams.selectedDatasetIds || [],

            // Value-based anomaly rules
            zScoreThreshold: savedParams.zScoreThreshold || prev.zScoreThreshold,
            spikeThreshold: savedParams.spikeThreshold || prev.spikeThreshold,

            // Time-based anomaly rules
            minEventDuration: savedParams.minEventDuration || prev.minEventDuration,
            ...
            // Peer comparison rules
            aggregationWindow: savedParams.aggregationWindow || prev.aggregationWindow,
            peerExceedThreshold: savedParams.peerExceedThreshold || prev.peerExceedThreshold,
          ...
          console.log('✅ Filter parameters loaded from saved data:', {
        ...
        console.error('❌ Failed to load saved filtering parameters:', error)
      }
    } else {
  ...
  const fetchAvailableDatasets = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const hasInitialFilteringParams = (initialExperimentRun as any)?.filteringParameters
  ...
  const onFilterParamChange = useCallback((key: keyof FilterParams, value: any) => {
    setFilterParams(prev => ({
      ...prev,
      [key]: value,
    }))
  ...
  const handleGenerateCandidates = async () => {
    ...
    try {
      const response = await fetch('https://python.yinchen.tw/api/v2/generate-candidates', {
        method: 'POST',
        headers: {
  ...
  const handleContinueLabeling = async () => {
    ...
    try {
      ...
      const eventsCreated = data.anomaly_events_created || data.candidate_count
...
錯誤詳情：${error instanceof Error ? error.message : String(error)}`
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Stage 1: Anomaly Candidate Generation Framework</CardTitle>
          <p className="text-sm text-muted-foreground">
            Multi-dimensional filtering methodology for systematic anomaly detection in occupancy
            sensing data
          </p>
        </CardHeader>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Select specific analysis datasets for anomaly candidate generation
          </p>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-200">
            <div className="space-y-1">
              <h4 className="text-sm font-medium text-blue-800">Total Data Pool Configuration</h4>
              <p className="text-xs text-blue-600">
                {selectedDatasets.length > 0 ? (
                  <>
                    Combined {selectedDatasets.length} dataset(s) into total data pool of{' '}
                    {selectedDatasets.reduce((sum, d) => sum + d.totalRecords, 0).toLocaleString()}{' '}
                    records ({selectedDatasets.reduce((sum, d) => sum + d.positiveLabels, 0)}{' '}
                    positive labels,{' '}
                    {(
                      selectedDatasets.reduce((sum, d) => sum + d.totalRecords, 0) -
                      selectedDatasets.reduce((sum, d) => sum + d.positiveLabels, 0)
                    ).toLocaleString()}{' '}
                    negative labels)
                  </>
                ) : (
                  <>
                    No datasets selected. Choose from {availableDatasets.length} available datasets
                    to form your total data pool.
                  </>
                )}
              </p>
            </div>
              ...
              {selectedDatasets.length > 0 && (
                <>
                  ...
                  <Badge variant="outline" className="text-orange-600 border-orange-300">
                    {selectedDatasets.reduce((sum, d) => sum + d.positiveLabels, 0)} positive
                  </Badge>
          ...
          {availableDatasets.length > 0 && (
            <div className="flex items-center gap-2 pb-2 border-b">
              <Button
                ...
                disabled={selectedDatasets.length === availableDatasets.length}
              >
                Select All
              </Button>
              <Button
              ...
              <div className="text-xs text-gray-500 ml-4">
                Click individual datasets below to select/deselect
              </div>
            </div>
          )}
          ...
          {isLoadingDatasets ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <div className="space-y-2">
                <p className="font-medium">No Analysis Datasets Available</p>
                <p className="text-sm">
                  Please ensure analysis datasets are created and available in the system.
                </p>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-96 overflow-y-auto">
              {availableDatasets.map(dataset => {
                const isSelected = filterParams.selectedDatasetIds.includes(dataset.id)
                return (
                  <Card
                    key={dataset.id}
                    ...
                    onClick={() => {
                      const newSelectedIds = isSelected
                        ? filterParams.selectedDatasetIds.filter(id => id !== dataset.id)
                        : [...filterParams.selectedDatasetIds, dataset.id]
                      setFilterParams(prev => ({
                        ...prev,
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="text-xs text-gray-600 space-y-1">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>{dataset.occupantType}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <span className="font-medium">Period:</span>
                            <span>
                              {new Date(dataset.startDate).toLocaleDateString()} -{' '}
                              {new Date(dataset.endDate).toLocaleDateString()}
                            </span>
                          </div>
                          <div className="flex items-center gap-1">
                          ...
                          <div className="flex items-center gap-1">
                            <span className="font-medium">Known Anomalies:</span>
                            <span className="text-orange-600">{dataset.positiveLabels}</span>
                          </div>
                        </div>

                        {/* Additional Info */}
                        {dataset.description && (
                          <div className="text-xs text-gray-500 italic">{dataset.description}</div>
                        )}
                      </div>
                    </CardContent>
                  </Card>
          ...
          {selectedDatasets.length > 0 && (
            <div className="mt-4 p-4 bg-gray-50 rounded-lg border">
              <h5 className="text-sm font-medium text-gray-800 mb-3">
                Total Data Pool Summary ({selectedDatasets.length} datasets selected)
              </h5>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-xs">
                <div className="text-center">
                  ...
                  <div className="text-gray-600">Total Data Pool Size</div>
                </div>
                <div className="text-center">
                  <div className="font-medium text-orange-600">
                    {selectedDatasets.reduce((sum, d) => sum + d.positiveLabels, 0)}
                  </div>
                  <div className="text-gray-600">Positive Labels</div>
                </div>
                <div className="text-center">
                  ...
                  <div className="text-gray-600">Unlabeled Labels</div>
                </div>
                <div className="text-center">
                  <div className="font-medium text-green-600">
                    {[...new Set(selectedDatasets.map(d => d.building))].length}
                  </div>
                  <div className="text-gray-600">Buildings</div>
                </div>
                <div className="text-center">
                  <div className="font-medium text-purple-600">
                    {[...new Set(selectedDatasets.map(d => d.occupantType))].length}
                  </div>
                  <div className="text-gray-600">Occupant Types</div>
                </div>
              </div>
            </div>
          )}
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Configure statistical thresholds and temporal constraints for anomaly identification
          </p>

          {/* Filtering Logic Explanation */}
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h6 className="font-medium text-blue-800 mb-2">Anomaly Detection Methodology</h6>
            <div className="text-sm text-blue-700 space-y-1">
              <p>
                • <strong>Z-Score Threshold:</strong> Statistical outlier detection for data points
                deviating from mean (σ {'>'}&#8194;
                {filterParams.zScoreThreshold})
              </p>
              <p>
                • <strong>Power Spike Detection:</strong> Identifies consumption patterns exceeding{' '}
                {filterParams.spikeThreshold}% above baseline average
              </p>
              <p>
                • <strong>Temporal Duration Filter:</strong> Excludes transient events lasting less
                than {filterParams.minEventDuration} minutes
              </p>
              <p>
                • <strong>Power Jump Analysis:</strong> Detects rapid consumption changes exceeding
                50% between adjacent time points
              </p>
            </div>
            ...
            <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded">
              <div className="text-xs text-amber-800">
                <strong>💡 Adaptive Parameter Recommendations:</strong>
                {selectedDatasets.length === 0 && ' Please select spatial domains for analysis'}
              ...
              {selectedDatasets.length > 0 && (
                <div className="text-xs text-amber-700 mt-1">
                  Expected candidate range: {selectedDatasets.length * 30} -{' '}
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            {selectedDatasets.length > 0 && (
              <div className="mb-3">
                ...
                >
                  🎯 Apply Adaptive Parameter Optimization ({selectedDatasets.length} spatial
                  domains)
                </Button>
              </div>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <h5 className="font-medium text-slate-600">Active Detection Parameters</h5>
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-gray-50 rounded">
                  <div className="text-sm font-medium">Z-Score Threshold</div>
                  <div className="text-lg">{filterParams.zScoreThreshold}σ</div>
                </div>
                <div className="text-center p-3 bg-gray-50 rounded">
                  <div className="text-sm font-medium">Spike Threshold</div>
                  <div className="text-lg">{filterParams.spikeThreshold}%</div>
                </div>
                <div className="text-center p-3 bg-gray-50 rounded">
                  <div className="text-sm font-medium">Min Duration</div>
                  <div className="text-lg">{filterParams.minEventDuration}min</div>
                </div>
                <div className="text-center p-3 bg-gray-50 rounded">
                  <div className="text-sm font-medium">Max Time Gap</div>
                  <div className="text-lg">{filterParams.maxTimeGap}min</div>
                </div>
              </div>
            </div>

            <div className="space-y-4">
              <h5 className="font-medium text-slate-600">Parameter Visualization</h5>
              <div className="grid grid-cols-2 gap-4">
                {/* Z-Score Chart */}
                <div className="flex flex-col items-center p-3 bg-white border rounded">
                  <div className="text-xs font-medium mb-2">Z-Score Threshold</div>
                  <D3ParameterChart
                    ...
                    height={60}
                  />
                  <div className="text-xs text-gray-500 mt-1">{filterParams.zScoreThreshold}σ</div>
                </div>

                {/* Spike Chart */}
                <div className="flex flex-col items-center p-3 bg-white border rounded">
                  <div className="text-xs font-medium mb-2">Power Spike Analysis</div>
                  <D3ParameterChart
                    parameterType="spike"
                ...
                {/* Duration Chart */}
                <div className="flex flex-col items-center p-3 bg-white border rounded">
                  <div className="text-xs font-medium mb-2">Temporal Duration</div>
                  <D3ParameterChart
                  ...
                  />
                  <div className="text-xs text-gray-500 mt-1">
                    {filterParams.minEventDuration} minutes minimum
                  </div>
                </div>

                {/* Time Gap Chart */}
                <div className="flex flex-col items-center p-3 bg-white border rounded">
                  <div className="text-xs font-medium mb-2">Temporal Continuity</div>
                  <D3ParameterChart
                  ...
                  />
                  <div className="text-xs text-gray-500 mt-1">
                    {filterParams.maxTimeGap} minutes maximum gap
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="pt-4 border-t">
            <Button
              ...
              size="lg"
              disabled={isGenerating || selectedDatasets.length === 0 || !experimentRun}
                ...
                <div className="text-center py-8">
                  ...
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 max-w-md mx-auto">
                    <p className="text-sm text-orange-800 mb-3">
                      Current statistical parameters yielded zero anomaly candidates. Parameter
                      adjustment recommendations:
                    </p>
                    <ul className="text-sm text-orange-700 list-disc list-inside space-y-1 text-left">
                      <li>
                        Reduce Z-Score threshold (current: σ ≥ {filterParams.zScoreThreshold})
                      </li>
                      <li>
                        Lower power spike threshold (current: ≥ {filterParams.spikeThreshold}% above
                        baseline)
                      </li>
                      <li>
                        Decrease minimum temporal duration (current: ≥{' '}
                        {filterParams.minEventDuration} minutes)
                      </li>
                      <li>Expand temporal or spatial scope for analysis</li>
                    </ul>
                  </div>
                  {experimentRun && (
                    <div className="text-sm text-gray-600 mt-4">
                      ...
                      <span className="ml-2">
                        {new Date(experimentRun.created_at).toLocaleString()}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      ...
                      <div className="text-sm text-gray-600">Detected Candidates</div>
                    </div>
                    {experimentRun && (
                      <div className="text-center">
                        <Badge
                          ...
                          className={labelingReady ? 'bg-green-600' : ''}
                        >
                          {labelingReady ? 'ANNOTATION READY' : experimentRun.status}
                        </Badge>
                        <div className="text-sm text-gray-600 mt-1">Processing Status</div>
                      </div>
                    )}
                    {selectedDatasets.length > 0 && (
                      <div className="text-center">
                        <div className="text-lg font-semibold text-green-600">
                          {selectedDatasets.length === 1
                            ? selectedDatasets[0].room
                            : `${selectedDatasets.length} spatial domains`}
                        </div>
                        <div className="text-sm text-gray-600">Analysis Scope</div>
                      </div>
                    )}
                    {selectedDatasets.length > 0 && (
                      <div className="text-center">
                        ...
                        <div className="text-sm text-gray-600">Sensor Records</div>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="font-medium text-green-800">
                          🎯 Anomaly Event Labels Successfully Generated
                        </span>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div className="bg-white p-3 rounded border">
                          <div className="font-medium text-green-700">Event Count</div>
                          <div className="text-lg font-bold text-green-600">
                            {labelingInfo.anomaly_events_created}
                          </div>
                        </div>
                        <div className="bg-white p-3 rounded border">
                          <div className="font-medium text-blue-700">Experiment ID</div>
                          <div className="text-xs font-mono text-blue-600 break-all">
                            {labelingInfo.experiment_run_id}
                          </div>
                        </div>
                        <div className="bg-white p-3 rounded border">
                          <div className="font-medium text-purple-700">Pipeline Status</div>
                      ...
                      <p className="text-sm text-green-700 mt-3">
                        ✅ All anomaly candidates successfully converted to annotation events and
                        persisted to database. Ready to proceed to Stage 2 for expert validation and
                  ...
                  {candidateCount > 0 && experimentRun && experimentRun.status === 'COMPLETED' && (
                    <div className="pt-4 border-t">
                      {labelingReady ? (
                        ...
                        <>
                          <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="font-medium text-blue-800">
                                Ready to Enter Manual Labeling Phase
                              </span>
                            </div>
                            <p className="text-sm text-blue-700">
                              Anomaly events are prepared. Click the button below to proceed to
                              Stage 2 for manual labeling review.
                            </p>
                          </div>
                          <Button
                            onClick={() => {
                              ...
                              currentUrl.searchParams.set('stage', 'stage-2')
                              router.push(
                          ...
                          >
                            ...
                            🏷️ Proceed to Stage 2 (Expert Labeling)
                          </Button>
                          <p className="text-xs text-gray-500 mt-2 text-center">
                            Will begin manual review and labeling of {candidateCount} anomaly events
                          </p>
                        </>
                      ) : (
                        ...
                        <>
                          <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                            <div className="flex items-center gap-2 mb-2">
                              <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse" />
                              <span className="font-medium text-green-800">
                                Ready to Create Anomaly Event Labels
                              </span>
                            </div>
                            <p className="text-sm text-green-700">
                              Candidate generation complete. Click the button below to convert{' '}
                              {candidateCount} candidates into anomaly event labels.
                              <br />
                              <span className="text-xs text-green-600">
                                ⏱️ This operation may take several minutes, please be patient
                              </span>
                            </p>
                          </div>
                          <Button
                            onClick={handleContinueLabeling}
                            disabled={isContinuingLabeling}
                            className="w-full bg-green-600 hover:bg-green-700 text-white"
                          >
                            {isContinuingLabeling ? (
                              <>
                                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                Creating anomaly event labels... Please wait
                              </>
                            ) : (
                              <>
                                <ArrowRight className="mr-2 h-4 w-4" />📝 Create Anomaly Event
                                Labels
                              </>
                            )}
                          </Button>
                          <p className="text-xs text-gray-500 mt-2 text-center">
                            Will create {candidateCount} anomaly events in database ready for
                            labeling
                            <br />
                            ⚠️ Operation takes up to 5 minutes, no retry mechanism
                          </p>
                        </>
                      )}
                    </div>
