    ...
    <div className="space-y-12">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-slate-700 max-w-3xl mx-auto">
          This interactive sandbox replicates Positive-Unlabeled learning scenarios, implementing
          both uPU and nnPU algorithms. It allows direct experimentation with class-prior estimation
          methods, model complexity, regularization, and feature dimensionality—mirroring the
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                >
                  ...
                  <span>
                    1. Experiment Setup – Control the synthetic data distribution and labeling
                    scenario
                  </span>
                </button>
                {experimentOpen && (
                ...
                {experimentOpen && <hr className="my-4 border-gray-200" />}
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <span>2. Model Configuration – Tune PU algorithm parameters</span>
                </button>
                {modelOpen && (
                  <div className="pl-6 space-y-4 pt-1">
                    <ModelConfigurationControls
                      ...
                      onMethodChange={setPriorEstimationMethod}
                      ...
                      onBlindsEffectSetup={handleBlindsEffectSetup}
                    />
                  </div>
                )}
                {modelOpen && <hr className="my-4 border-gray-200" />}
              </div>
              ...
              <div className="pl-6 space-y-4">
                ...
                <div className="pt-2">
                  ...
                  >
                    Use this configuration on real testbed data →
                  </a>
                </div>
              </div>
            </CardContent>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {results && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <RiskCurveChart results={results} algorithm={algorithm} />

              {/* 右邊：Current Configuration */}
              <Card className="bg-slate-50 border border-slate-200">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm text-slate-700">Current Configuration</CardTitle>
                </CardHeader>
                <CardContent className="text-xs space-y-1">
                  <div className="grid grid-cols-2 gap-6">
                    {/* 左邊：輸入的數值 */}
                    <div className="space-y-2">
                      <div className="font-medium text-slate-800 mb-2">Input Parameters</div>
                      <div className="space-y-1">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{currentConfig.distribution}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Dimensions:</span>
                          <span className="font-medium">{currentConfig.dimensions}D</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Sample Size:</span>
                          <span className="font-medium">{currentConfig.sampleSize}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{currentConfig.hiddenSize}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {results.metrics && (
                      <div className="space-y-2">
                        <div className="font-medium text-slate-800 mb-2">Results</div>
                        <div className="space-y-1">
                          {/* Est. Prior */}
                          <div className="flex justify-between">
                            <span>Est. Prior:</span>
                            <span className="font-medium">
                              {(results.metrics.estimatedPrior * 100).toFixed(2)}%
                            </span>
                          </div>
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              {results.metrics.trainingErrorRate.toFixed(2)}
                            </span>
                          </div>

                          {/* Model Status */}
                          {(() => {
                            const { errorRate, trainingErrorRate, estimatedPrior } = results.metrics

                            // 計算先驗估計誤差（假設真實先驗為0.3）
                            const truePrior = 0.3
                            const priorError = Math.abs(estimatedPrior - truePrior)

                            // 優先特赦條款：如果核心指標很棒，直接判定為健康
                            const hasExcellentCoreMetrics = priorError < 0.05 && errorRate < 0.05

                            // 次要判斷：過擬合檢測
                            const isSevereOverfitting =
                              !hasExcellentCoreMetrics &&
                              errorRate > trainingErrorRate * 3 &&
                              trainingErrorRate < 0.05

                            let statusIcon: string
                            let statusText: string

                            if (hasExcellentCoreMetrics) {
                              statusIcon = '🟢'
                              statusText = 'Healthy'
                            } else if (isSevereOverfitting) {
                              statusIcon = '🔴'
                              statusText = 'Severe Overfitting'
                            } else {
                              statusIcon = '🟢'
                              statusText = 'Healthy'
                            }
