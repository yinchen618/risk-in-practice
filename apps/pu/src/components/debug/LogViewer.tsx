...
export function LogViewer() {
  ...
  const recentApiLogs = logs.filter(
    log => log.source?.includes('API') && Date.now() - new Date(log.timestamp).getTime() < 30000 // 最近30秒
  )
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <div className="h-64 border rounded-md p-2 overflow-auto">
            {recentApiLogs.length === 0 ? (
              <p className="text-sm text-muted-foreground">暫無 API 調用記錄</p>
            ) : (
                    ...
                    <div key={idx} className="text-xs font-mono bg-slate-50 p-2 rounded">
                      ...
                      {log.data && (
                        <details className="mt-1">
                          <summary className="text-slate-500 cursor-pointer">詳細資料</summary>
        ...
        <div className="text-sm text-muted-foreground">
          總日誌條數: {logs.length} | API 相關: {logs.filter(l => l.source?.includes('API')).length}
        </div>
      </CardContent>
    </Card>
  )
