...
class CandidateCalculationService:
    ...
    async def _calculate_top_devices(
        ...
        # 導入 data_loader 來獲取房間映射
        from services.data_loader import data_loader
        device_room_mapping = data_loader.get_device_room_mapping()

        def building_code(building_name):
            if building_name == "Building A":
                return "Building A"
            elif building_name == "Building B":
                return "Building B"
            return building_name or "unknown"

        top_devices = [
            {
            "deviceNumber": device,
            "estimatedAnomalies": count,
            "roomInfo": device_room_mapping.get(device, {
                "building": "unknown",
                "room": "unknown",
                "floor": "unknown"
            }),
            "displayName": f"{building_code(device_room_mapping.get(device, {}).get('building', 'unknown'))} {device_room_mapping.get(device, {}).get('floor', 'unknown')}F {device_room_mapping.get(device, {}).get('room', 'unknown')}"
            }
