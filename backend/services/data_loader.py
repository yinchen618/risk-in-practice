...
class DataLoaderService:
    ...
    def _get_cached_raw_dataframe(self, cache_key: str) -> pd.DataFrame:
        ...
        try:
            # Load real data from database - use thread pool to avoid event loop conflicts
            ...
            def run_async_in_thread():
                ...
                try:
                    result = loop.run_until_complete(self._load_real_data_async())
                    return result
                finally:
                    loop.close()
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                # 實際資料量較大時，載入可能超過 30 秒，放寬為 180 秒
    ...
    async def get_raw_dataframe(self, 
        ...
        # 若沒有資料，直接返回，避免後續欄位過濾拋出 KeyError
        if df.empty:
            return df
