...
class DataLoaderService:
    ...
    async def _load_raw_data_from_db(self) -> pd.DataFrame:
        """從資料庫載入原始電表數據"""
        try:
            # 從資料庫獲取最近30天的電表數據
            end_date = datetime.utcnow()
            ...
            # 這裡可以根據需要調整查詢邏輯
            logs = await db_manager.get_ammeter_logs(limit=10000)
            
            if not logs:
                logger.warning("未找到電表數據，生成模擬數據")
                return self._generate_mock_data()
            
            # 轉換為 DataFrame
            data_rows = []
            for log in logs:
                if log.success and log.power is not None:
                    data_rows.append({
                        'deviceNumber': log.deviceNumber,
                        'timestamp': log.createdAt,
                        'power': log.power,
                        ...
                        'switchState': log.switchState or 0,
                        'networkState': log.networkState or 0
                    })
            
            if not data_rows:
                logger.warning("沒有有效的電表數據，生成模擬數據")
                return self._generate_mock_data()
            ...
            logger.info(f"載入了 {len(df)} 筆電表數據，涵蓋 {df['deviceNumber'].nunique()} 個設備")
            return df
            
        except Exception as e:
            logger.error(f"載入資料庫數據失敗: {e}")
            return self._generate_mock_data()
    ...
    async def _load_raw_data_from_db(self) -> pd.DataFrame:
        ...
        try:
            # 從資料庫獲取最近3天的電表數據
            end_date = datetime.utcnow()
            start_date = end_date - timedelta(days=3)  # 改為3天
            
            # 獲取真實的電表日誌數據
            logs = await db_manager.get_ammeter_logs(limit=50000)
            
            if not logs:
                logger.warning("資料庫中未找到電表數據")
                return pd.DataFrame()
            
            # 轉換為 DataFrame，只包含最近3天的數據
            data_rows = []
            for log in logs:
                if (log.success and 
                    log.power is not None and 
                    ...
                    data_rows.append({
                        ...
                        'timestamp': log.createdAt,
                        'power': log.power,
                        'voltage': log.voltage or 220.0,
                        'current': log.currents or 0.0,
                        'battery': log.battery or 85.0,
                        'switchState': log.switchState or 1,
                        'networkState': log.networkState or 1
                    })
            
            if not data_rows:
                logger.warning("最近3天沒有有效的電表數據")
                return pd.DataFrame()
            ...
            logger.info(f"載入了 {len(df)} 筆真實電表數據，涵蓋 {df['deviceNumber'].nunique()} 個設備")
            logger.info(f"日期範圍: {df['timestamp'].min()} 到 {df['timestamp'].max()}")
            return df
            ...
            return pd.DataFrame()
        """生成模擬電表數據用於開發測試"""
        logger.info("生成模擬電表數據...")
        
        # 生成3個設備，每個設備30天的數據，每小時一筆
        devices = ['METER001', 'METER002', 'METER003']
        ...
        for device in devices:
            ...
            base_power = 2.5 + np.random.normal(0, 0.5)  # 基本功率
            
            while current_time < end_time:
                # 模擬日常用電模式
                hour = current_time.hour
                if 6 <= hour <= 9 or 18 <= hour <= 22:  # 早晚用電高峰
                    power_multiplier = 1.5 + np.random.normal(0, 0.3)
                elif 10 <= hour <= 17:  # 白天正常用電
                    power_multiplier = 1.0 + np.random.normal(0, 0.2)
                else:  # 夜間低用電
                    power_multiplier = 0.3 + np.random.normal(0, 0.1)
                
                # 偶爾生成異常值（5%機率）
                if np.random.random() < 0.05:
                    power_multiplier *= 3 + np.random.normal(0, 1)
                
                power = max(0, base_power * power_multiplier)
                
                data_rows.append({
                    'deviceNumber': device,
                    'timestamp': current_time,
                    'power': round(power, 2),
                    'voltage': round(220 + np.random.normal(0, 5), 1),
                    'current': round(power / 220, 2),
                    'battery': round(85 + np.random.normal(0, 10), 1),
                    'switchState': 1,
                    'networkState': 1
                })
        ...
        df = pd.DataFrame(data_rows)
        logger.info(f"生成了 {len(df)} 筆模擬數據")
        return df
    
    def _load_mock_data_sync(self) -> pd.DataFrame:
        """同步版本的模擬數據生成，避免 asyncio 問題"""
        logger.info("生成模擬電表數據...")
