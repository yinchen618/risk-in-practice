...
def setup_chinese_font_advanced():
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            if fonts:
                font_path = fonts[0].split(':')[0]
                font_name = os.path.basename(font_path).split('.')[0]
                plt.rcParams['font.sans-serif'] = [font_name] + plt.rcParams['font.sans-serif']
                print(f"✅ 使用系統中文字體: {font_name}")
                return True
    except:
        pass
    ...
    chinese_fonts = [
        ...
        'Liberation Sans', 'Ubuntu', 'FreeSans'
    ]
    ...
    # 方法3: 使用系統默認字體並設置 Unicode 支持
    plt.rcParams['font.sans-serif'] = ['DejaVu Sans'] + plt.rcParams['font.sans-serif']
    print("⚠️  使用 Unicode 支持")
...
def visualize_blinds_effect(result: Dict, config_name: str, analysis: Dict):
    ...
    plt.scatter(p_x, p_y, c='red', s=50, alpha=0.7, label='Positive Samples (P)')
    plt.scatter(u_x, u_y, c='gray', s=30, alpha=0.5, label='Unlabeled Samples (U)')
    ...
    # 添加標題和指標（純英文）
    title = f'nnPU Blinds Effect: {config_name}\n'
    title += f'Smoothness: {analysis["smoothness"]:.3f}, '
    title += f'Oscillations: {analysis["oscillations"]}, '
    title += f'Blinds Score: {analysis["blinds_score"]}, '
    title += f'Y Range: {analysis["y_range"]:.3f}'
    ...
    # 保存圖片
    filename = f'nnpu_gaussian_english_{config_name.replace(" ", "_").replace("=", "_")}.png'
...
def test_gaussian_overfitting_configurations():
    ...
    gaussian_configs = [
        {
            "name": "Gaussian_Extreme_LearningRate_ManyNeurons_NoReg",
            "params": {
        ...
        {
            "name": "Gaussian_Extreme_LearningRate_MoreNeurons_NoReg",
            "params": {
                "distribution": "gaussian",
                "dims": 2,
                "n_p": 50,
        ...
        {
            "name": "Gaussian_HighLearningRate_ExtremeNeurons_NoReg",
            "params": {
                "distribution": "gaussian",
                "dims": 2,
                "n_p": 50,
        ...
        {
            "name": "Gaussian_Extreme_LearningRate_ManyNeurons_LowPrior",
            "params": {
                "distribution": "gaussian",
                "dims": 2,
                "n_p": 50,
        ...
        {
            "name": "Gaussian_HighLearningRate_ManyNeurons_HighPrior",
            "params": {
                "distribution": "gaussian",
                "dims": 2,
                "n_p": 50,
        ...
        {
            "name": "Gaussian_Extreme_LearningRate_MoreNeurons_Imbalanced",
            "params": {
                "distribution": "gaussian",
                "dims": 2,
                "n_p": 30,
        ...
        {
            "name": "Gaussian_HighLearningRate_ExtremeNeurons_ExtremeImbalanced",
            "params": {
                "distribution": "gaussian",
                "dims": 2,
                "n_p": 20,
        ...
        {
            "name": "Gaussian_Extreme_LearningRate_ManyNeurons_ExtremeLowPrior",
            "params": {
                "distribution": "gaussian",
                "dims": 2,
                "n_p": 50,
    ...
    with open('gaussian_overfitting_english_results.json', 'w', encoding='utf-8') as f:
